...
program
  ...
  .action(async (options) => {
    try {
      ...
      if (options.calibreLibrary)
        config.calibreLibraryPath = options.calibreLibrary;
...
program
  ...
  .action(async (options) => {
    const sampleConfig: SyncConfig = {
      ...
      template:
        "{#genre} - {author_sort} - {series:|| }{series_index:|| - }{title} ({id})",
...
async function validateConfig(config: SyncConfig): Promise<void> {
  ...
  try {
    ...
    errors.push(
      `Calibre library path does not exist: ${config.calibreLibraryPath}`,
    );
  }
  ...
  try {
    ...
    errors.push(
      `Calibre metadata.db not found in: ${config.calibreLibraryPath}`,
    );
  }
